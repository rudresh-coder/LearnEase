...
chrome.storage.local.get([STICKY_NOTES_KEY], data => {
  ...
  const url = siteWide ? window.location.hostname : window.location.hostname + window.location.pathname;
  callback(notes[url] || []);
});
}
...
noteDiv.style.background = note.color || "#fffbe7";
...
noteDiv.style.zIndex = note.zIndex || 1;
...
noteDiv.tabIndex = 0; 
noteDiv.setAttribute("aria-label", note.title || "Sticky Note");
...
titleInput.value = note.title || "Sticky Note";
titleInput.placeholder = "Title";
titleInput.style.width = "70%";
titleInput.style.fontWeight = "bold";
titleInput.style.border = "none";
titleInput.style.background = "transparent";
titleInput.style.color = "#f59e42";
titleInput.style.fontSize = "1em";
titleInput.style.marginRight = "6px";
...
colorInput.value = note.color || "#fffbe7";
colorInput.title = "Pick note color";
...
colorInput.oninput = function() {
  notes[idx].color = colorInput.value;
  noteDiv.style.background = colorInput.value;
  updateNotes([...notes]);
};
...
noteDiv.onmousedown = function() {
  // Only bring to front if not minimized
  if (!note.minimized) {
    getStickyNotes(allNotes => {
      const maxZ = getMaxZIndex(allNotes);
      if (note.zIndex !== maxZ + 1) {
        notes[idx].zIndex = maxZ + 1;
...
noteDiv.addEventListener("keydown", (e) => {
  ...
  if ((e.key === "Delete" || e.key === "Backspace") && document.activeElement === noteDiv) {
    notes.splice(idx, 1);
    updateNotes([...notes]);
    renderStickyNotes();
    e.preventDefault();
  ...
  // Minimize/Restore: M
  if (e.key.toLowerCase() === "m" && document.activeElement === noteDiv) {
    notes[idx].minimized = !notes[idx].minimized;
    updateNotes([...notes]);
    renderStickyNotes();
    e.preventDefault();
  }
  // Focus textarea: Enter
  if (e.key === "Enter" && document.activeElement === noteDiv) {
    textarea.focus();
    e.preventDefault();
  }
});
...
document.querySelectorAll(".learnease-sticky-note, .learnease-sticky-note-minimized").forEach(e => e.remove());
getStickyNotes(notes => {
  notes.forEach((note, idx) => {
    let noteDiv;
    if (!note.minimized) {
      noteDiv = createStickyNote(note, idx, notes, updated => {
        saveStickyNotes(updated);
      });
    } else {
      // Minimized icon
      noteDiv = document.createElement("div");
      noteDiv.className = "learnease-sticky-note-minimized";
      noteDiv.style.position = "fixed";
      noteDiv.style.left = note.x + "px";
      noteDiv.style.top = note.y + "px";
      noteDiv.style.width = "32px";
      noteDiv.style.height = "32px";
      noteDiv.style.background = "#f59e42";
      ...
      noteDiv.innerText = "🗒️";
      noteDiv.title = "Restore Sticky Note";
      noteDiv.onclick = () => {
...
function getMaxZIndex(notes) {
  return notes.length ? Math.max(...notes.map(n => n.zIndex || 1)) : 1;
}
  ...
  if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === "n") {
    getStickyNotes(notes => {
      notes.push({
        ...
        title: "Sticky Note",
        color: "#fffbe7",
        x: 120 + 30 * notes.length,
        y: 120 + 30 * notes.length,
        width: 200,
        height: 120,
        minimized: false,
        zIndex: getMaxZIndex(notes) + 1
